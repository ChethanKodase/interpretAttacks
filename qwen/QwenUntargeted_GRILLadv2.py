...
for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargeted_GRILLadv2.py --attck_type grill_adv2 --desired_norm_l_inf 0.004 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargeted_GRILLadv2.py --attck_type grill_adv2 --desired_norm_l_inf 0.0035 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargeted_GRILLadv2.py --attck_type grill_adv2 --desired_norm_l_inf 0.0045 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
...
'''
FIXED VERSION — see accompanying notes for what changed and why.

Same launch commands as before, e.g.:
...
export PYTHONNOUSERSITE=1
for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargeted_GRILLadv2.py --attck_type grill_adv2 --desired_norm_l_inf 0.002 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
...
export PYTHONNOUSERSITE=1
for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargeted_GRILLadv2.py --attck_type grill_adv2 --desired_norm_l_inf 0.0025 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
...
export PYTHONNOUSERSITE=1
for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargeted_GRILLadv2.py --attck_type grill_adv2 --desired_norm_l_inf 0.003 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
...
def wasserstein_distance(tensor_a, tensor_b):
    a = torch.flatten(tensor_a).float()
    b = torch.flatten(tensor_b).float()
...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen2.5-VL original-image-space BSA attack (precision-fixed)"
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Forward-pass dtype for the Qwen model. bfloat16 matches the "
            "original script and Qwen's recommended inference dtype. "
            "float16 gives finer precision near cos()=1 (closer to the "
            "LLaVA fp16 setup) but can risk overflow on some Qwen layers; "
            "float32 is safest/most precise but ~2x memory and slower. "
            "The loss-side float32 upcast fix applies regardless of this "
            "setting."
        ),
    )
