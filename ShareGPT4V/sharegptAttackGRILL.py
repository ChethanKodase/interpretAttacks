...
python ShareGPT4V/sharegptAttackGRILLadv.py --attck_type grillAdv --desired_norm_l_inf 0.01 --learningRate 0.001 --num_steps 1000 --attackSample 0 --AttackStartLayer 0 --numLayerstAtAtime 1
...
for ATTACK_SAMPLE in $(seq 1 50); do
    python ShareGPT4V/sharegptAttackGRILL.py --attck_type grill --desired_norm_l_inf 0.001 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python ShareGPT4V/sharegptAttackGRILL.py --attck_type grill --desired_norm_l_inf 0.002 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python ShareGPT4V/sharegptAttackGRILL.py --attck_type grill --desired_norm_l_inf 0.003 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python ShareGPT4V/sharegptAttackGRILL.py --attck_type grill --desired_norm_l_inf 0.004 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python ShareGPT4V/sharegptAttackGRILL.py --attck_type grill --desired_norm_l_inf 0.005 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python ShareGPT4V/sharegptAttackGRILL.py --attck_type grill --desired_norm_l_inf 0.0015 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python ShareGPT4V/sharegptAttackGRILL.py --attck_type grill --desired_norm_l_inf 0.0025 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python ShareGPT4V/sharegptAttackGRILL.py --attck_type grill --desired_norm_l_inf 0.0035 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python ShareGPT4V/sharegptAttackGRILL.py --attck_type grill --desired_norm_l_inf 0.0045 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
...
# ----------------------------
def cos(a, b):
    # Cast up to fp32 before the sum/square reductions downstream (in
    # get_grill_cos_lossNew etc.) accumulate across ~57 hidden-state terms.
    # Those reductions can reach magnitudes that overflow fp16's ~65504 max
    # once squared, which silently produces inf -> nan and permanently
    # poisons the Adam optimizer state. The cast is differentiable, so
    # gradients still flow back into the (already-fp32) delta being
    # optimized; only the loss bookkeeping itself becomes numerically safe.
    a = a.reshape(-1)#.float()
    b = b.reshape(-1)#.float()
...
def get_bsa_loss(outputs, outputsN, startPos=None, endPos=None):
    """
    Token-wise cosine similarity over ShareGPT4V (LLaMA/Vicuna) hidden states.
...
def get_grill_cos_loss(outputs_adv, outputs_clean, acts_adv, acts_clean):
    loss = 0.0
    losses = []
    ...
    agg = losses_tensor.sum()*loss
    return agg


def get_grill_cos_lossNewEps(outputs_adv, outputs_clean, acts_adv, acts_clean):
    loss = 0.0
    epsCorr = 1e-8
    losses = []
    for h_adv, h_clean in zip(
...
def get_grill_cos_lossNewSq(outputs_adv, outputs_clean, acts_adv, acts_clean):
    loss = 0.0
    losses = []
    for h_adv, h_clean in zip(
        outputs_adv.hidden_states,
        ...
        loss = loss + ((1.0-cos(h_adv, h_clean))**2)**(1/2)
        losses.append(loss)

    # Vision hidden-state distortions
    for v_adv, v_clean in zip(acts_adv, acts_clean):
        loss = loss + ((1.0-cos(v_adv, v_clean))**2)**(1/2)
        losses.append(loss)
...
def get_grill_cos_lossNewSqClmp(outputs_adv, outputs_clean, acts_adv, acts_clean):
    loss = 0.0
    epsCorr = 1e-4
    ...
    for h_adv, h_clean in zip(
        ...
        loss = loss +  ((1.0-cos(h_adv, h_clean) + epsCorr)**2)**0.5 
        print("cos(h_adv, h_clean)", cos(h_adv, h_clean))
        losses.append(loss)

    # Vision hidden-state distortions
    for v_adv, v_clean in zip(acts_adv, acts_clean):
        loss = loss + ((1.0-cos(v_adv, v_clean) + epsCorr)**2)**0.5  
        print("cos(h_adv, h_clean)", cos(h_adv, h_clean))
        losses.append(loss)
...
def get_grill_cos_lossNewAbs(outputs_adv, outputs_clean, acts_adv, acts_clean):
    loss = 0.0
    losses = []
    for h_adv, h_clean in zip(
        outputs_adv.hidden_states,
        ...
        loss = loss + abs((1.0-cos(h_adv, h_clean))**2)
        losses.append(loss)

    # Vision hidden-state distortions
    for v_adv, v_clean in zip(acts_adv, acts_clean):
        loss = loss + abs((1.0-cos(v_adv, v_clean))**2)
        losses.append(loss)
...
def sharegpt_preprocess_differentiable(
    ...
    # dtype internally, but then casts the *output* image_features back to
    # match whatever dtype `images` had going in (see clip_encoder.py:
    # `.to(images.dtype)`). If we hand it fp32, encode_images() ends up
    # feeding fp32 activations into the fp16 mm_projector Linear and
    # crashes with a dtype mismatch. So we must hand the model fp16 input
    # directly here, exactly like share4v.eval.run_share4v does with
    # `.half()`. The cast is still differentiable, so gradients flow back
    # into the fp32 delta being optimized.
...
def adam_attack_original_space(
    ...
    # ShareGPT4V loads in fp16; images fed to the model must match (see the
    # note in sharegpt_preprocess_differentiable). Keep delta itself in
    # fp32 for stable Adam steps; only the tensor handed to the model gets
    # cast to fp16, and gradients cast back into delta automatically.
    ...
    for step in range(num_steps):
        ...
        loss = -1 * get_grill_cos_loss(outputs, outputsN, acts, actsN)
