...
python llava_attack/llavaAttackGRILL2.py --attck_type bsa --desired_norm_l_inf 0.01 --learningRate 0.001 --num_steps 1000 -attackSample 0 --AttackStartLayer 0 --numLayerstAtAtime 1
...
for ATTACK_SAMPLE in $(seq 1 50); do
    python llava_attack/llavaAttackGRILL2.py --attck_type grill2 --desired_norm_l_inf 0.005 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done

for ATTACK_SAMPLE in $(seq 1 50); do
    python llava_attack/llavaAttackGRILL2.py --attck_type grill2 --desired_norm_l_inf 0.004 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done

for ATTACK_SAMPLE in $(seq 1 50); do
    python llava_attack/llavaAttackGRILL2.py --attck_type grill2 --desired_norm_l_inf 0.003 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done

for ATTACK_SAMPLE in $(seq 1 50); do
    python llava_attack/llavaAttackGRILL2.py --attck_type grill2 --desired_norm_l_inf 0.002 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done

for ATTACK_SAMPLE in $(seq 1 50); do
    python llava_attack/llavaAttackGRILL2.py --attck_type grill2 --desired_norm_l_inf 0.001 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE
done
...
def get_grill_vis_lan_loss(acts, actsN, outputs, outputsN):

    loss = 0.0
    for h, hn in zip(acts, actsN):
        cos_per_token = F.cosine_similarity(h, hn, dim=-1)
        loss = loss + cos_per_token.mean()
    ...
    meanCosPerTocken = cos_per_token.mean()#/ cos_per_token.shape[0]

    #return (1-loss)**2 * (1 - meanCosPerTocken)**2
    return loss * meanCosPerTocken

'''def get_grill_vis_lan_loss(acts, actsN, outputs, outputsN, eps=1e-6, max_val=1e4):
    loss = torch.zeros((), device=acts[0].device, dtype=torch.float32)

    for h, hn in zip(acts, actsN):
        cos_per_token = F.cosine_similarity(
            h.float(), hn.float(), dim=-1, eps=eps
        )
        cos_per_token = torch.nan_to_num(cos_per_token, nan=0.0, posinf=1.0, neginf=-1.0)
        loss = loss + cos_per_token.mean()
    ...
    meanCosPerTocken = cos_per_token.mean()

    # normalize by number of terms
    n_terms = len(acts) + len(outputs.hidden_states)
    #loss = loss / n_terms

    # correction / stabilization term: clamp before squaring
    a = torch.clamp(1.0 - loss/n_terms, min=-max_val, max=max_val)
    b = torch.clamp(1.0 - meanCosPerTocken, min=-max_val, max=max_val)
...
def adam_attack_original_space(
    ...
    for step in range(num_steps):
        ...
        loss = 1 * get_grill_vis_lan_loss(acts, actsN, outputs, outputsN) 
